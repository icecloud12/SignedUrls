...
use crate::{
    ...
    models::request_models::{
        RequestQueryParamsV2, UploadRequestDocument, UploadRequestWithBucketDocument,
    },
    network::{db_connection::DATABASE, DbCollection},
    signed_url::actions::{
        save_files_to_directory, validate_signed_url_v1, validate_signed_url_v2, ActionTypes,
        UploadActionTypes,
    },
};
use axum::{
...
pub async fn process_signed_url_upload_request_v1(
    ...
    if validate_signed_url_v1(
        collected_params,
        ActionTypes::UPLOAD_V1.to_string().as_str(),
    )
    .await
    {
        ...
        let project = db
            ...
            .unwrap()
            .unwrap();
        let save_files_to_directory_result = save_files_to_directory(
            &request._id.to_hex(),
            &project._id.to_hex(),
            request.target,
...
pub async fn process_signed_url_upload_request_v2(
    ...
    if validate_signed_url_v2(None, query, ActionTypes::UPLOAD_V2).await {
        ...
        match db
            .collection::<UploadRequestWithBucketDocument>(
            ...
            .await
        {
            ...
            Ok(mut cursor_document) => {
                ...
                match cursor_document.advance().await {
                    Err(_) => return (StatusCode::SERVICE_UNAVAILABLE).into_response(),
                    Ok(_) => match cursor_document.with_type().deserialize_current() {
                        Ok(document) => {
                            let UploadRequestWithBucketDocument {
                                _id: request_id,
                                target,
                                options,
                                bucket: BucketDocument { _id: bucket_id, .. },
                                ..
                            } = document;
                            ...
                            let save_files_to_directory_result = save_files_to_directory(
                                ...
                                options,
                                multipart,
                                UploadActionTypes::try_from(ActionTypes::UPLOAD_V2).unwrap(),
                            )
                            .await;
                            match save_files_to_directory_result {
                                Ok(res) => {
